...
- [x] [PN-18] Provide a Docker Compose example that persists the SQLite database on an external volume and documents how to run it end-to-end.
  - Resolved: Added `docker-compose.yaml` with a named Docker volume and sample `.env.pinguin.example`, documented compose usage in README, and verified go test/go vet run cleanly.
