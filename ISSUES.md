...
- [ ] [PN-100] Add a front-end.
  ...
  Plan tasks:
    ...
    - [x] PN-100.H — Cover dashboard auth guards in Playwright (load `/dashboard.html` while logged out, exercise BroadcastChannel logout redirect) so route protection is verified end-to-end. (Added two dashboard specs to assert unauthenticated loads jump to the landing page and BroadcastChannel `logged_out` events trigger redirects.)
    - [x] PN-100.I — Expand dashboard Playwright coverage for status filters and API failure toasts (mock 500 responses, ensure disabled buttons + error copy) to lock down error handling UX. (Dev server now respects status query params and exposes failure flags, and new e2e specs cover filtering plus load/reschedule/cancel error toasts.)
    - [x] PN-100.J — Simulate full landing sign-in handshake in tests (nonce fetch, Google credential callback, `/auth/google` exchange) to prove the Alpine auth controller’s happy path. (Landing Playwright spec now exercises the full GIS → `/auth/nonce` → `/auth/google` flow and asserts redirect to the dashboard.)
