...
- [ ] [PG-103] add a flag (matched by an enviornment variables) that disables web interface. when the web interface is dsiabled it doesnt chech for the environment variables/flags required for web-interface functioning, such as ADMINS, GOOGLE_CLIENT_ID, HTTP_LISTEN_ADDR, HTTP_ALLOWED_ORIGINS, HTTP_STATIC_ROOT
